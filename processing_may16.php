<?php
	...
	if( isset($_POST['payingBalanceNow']) ){
		...
		$sql = "SELECT * FROM invoices WHERE customerid='$balCustId' AND transaction='In' AND fullPayment!='Full' AND status='1'";
		...
		for( $i=0; $i<=count($data); $i++){			
			...
			if( $data[$i]['pending'] == 0 && ($data[$i]['prepending'] == $data[$i]['paid']) ){
				if( $clearance_payingAmount > 0 ){
					$sql = "INSERT INTO invoices (refId, finaltotal, partialPayment, transaction, status, login, date, pendingAmount, customer, customerid) VALUES('$clearance_refId', '', '$clearance_payingAmount', 'In', '1', '$login', '".date('Y-m-d')."', '$finalPendingBillAmount2', '$customer', '$customer')";
