				...
				<table class='display nowrap' id='datatables' cellspacing="0" width="100%">
					...
					<tbody>
						<?php 
							...
							$sql = "SELECT * FROM invoices WHERE customerid='$custId' AND status='1' AND transaction='In' AND fullPayment!='Full' AND login='$login'";
							$get = mysqli_query($db, $sql);
								...
								while( $res = mysqli_fetch_array($get) ){
									...
									if( $refId > 0 ){
										...
										$customerInfo = $getTransactionDetails[0].", <br>".$getTransactionDetails[1]."<br><i class='text-danger'>Reference of #$refId</i>$custGivenAmount";
	...
	$(document).ready(function() {
		...
		$("#payBalance").click(function(){
			...
			if( payingBalanceNow <= payingBalanceNowActualAmount ){
				...
				finalId = balCustId;
				$.post('processing.php', {payingBalanceNow:payingBalanceNow, balCustId:balCustId}, function(res){		
					...
					for (const [key, value] of Object.entries(result[0])) {
						...
						arr.push({id:key, prepending:value, pending:mypen.pending, paid:mypen.paid, customerId:balCustId, amountReceivedFromCustomer:payingBalanceNow});					
					}		
					console.log(arr);		
					let clearedItems = [];
		...
		$(document).on('click', '#confirmPayments_btn', function(){
			...
			$.post('processing.php', {clearningPendingBalance:finalList, forCustomer:finalId, customerId:finalId}, function(res){	
