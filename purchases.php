		  ...
		  <div class="panel-body">
			...
			<form action='purchases.php' method='get'>
				<div class='form-group col-md-2'>
					<input type='text' name='date1' autocomplete="off" class='form-control datepicker' placeholder='From Date' />
				</div>
				<div class='form-group col-md-2'>
					<input type='text' name='date2' autocomplete="off" class='form-control datepicker' placeholder='To Date' />
				</div>
				...
				<table class='display' id='datatables' cellspacing="0" width="100%">
					<thead>
						<th>Id</th>
						<th>Type</th>
						<th>Customer</th>
							...
							if( isset($_GET['submit']) ){
								...
								if( !empty($date1) && !empty($date2) ){
									if( $login == "admin" ){
										...
										$qry = "WHERE (`date` BETWEEN '$date1' AND '$date2') AND status='1' AND transaction='Out' AND returnStatus='0' AND login='$login'";
									}	
								}else{
									if( $login == "admin" ){
										$qry = "WHERE status='1' AND transaction='Out' AND returnStatus='0'";
									}else{
										$qry = "WHERE status='1' AND transaction='Out' AND returnStatus='0' AND login='$login'";
									}	
								}
							}else{
								if( $login == "admin" ){
									...
									$qry = "WHERE date='".date('Y-m-d')."' AND status='1' AND transaction='Out' AND returnStatus='0' AND login='$login'";
								}
							}	
