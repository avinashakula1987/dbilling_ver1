			...
			$( ".customers_dropdown" ).autocomplete({
			   source: customersinformation,
				select: function( event, ui ) {
					...
					$('#vehicle').focus();
					localStorage.setItem("choosenCustomerId", ui.item.id);
					localStorage.setItem("choosenCustomerName", ui.item.label);
				},
				minLength:2	
			});	
			
			var purchasersinformation = <?php include("dynamic_purchasers.php"); ?>;
			
			$( ".purchasers_dropdown" ).autocomplete({
			   source: purchasersinformation,
				select: function( event, ui ) {
					$( this ).val( undefined );
					$( this ).attr( 'hiddenid',undefined );
					$( '#mobile' ).val( ui.item.mobile );
