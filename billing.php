		  ...
		  <div class="panel-body">
			...
			<div class='well stockform billingblock table-responsive'>
				...
				<div class='row'>
					...
					<!-- <div class='form-group col-md-3 col-sm-6 col-xs-12'>
						<input type='text' id='hamaliCharges' class='form-control' placeholder='Hamali Charges' />
					</div>	 -->
					<div class='form-group col-md-3 col-sm-6 col-xs-12'>
						<input type='text' id='openingBalance' readonly disabled style="display:none" class='form-control' placeholder='Opening Balance' />
					</div>
			...
			<div class='well'>
				<div class='row'>
					...
					<div class='form-group col-md-3 col-sm-3 col-xs-3'>
						<input type='text' placeholder='Pending' value='0' disabled class='form-control' id='finalPendingBillAmount' />
					</div>
				</div>
			</div>	
...
<script>
	$(document).ready(function(){
		function isGenuineCustomer(){
			let choosenCustomerId = localStorage.getItem('choosenCustomerId');
			let choosenCustomerName = localStorage.getItem('choosenCustomerName');
			let id = $('.customers_dropdown').attr('hiddenid');
			let name = $('.customers_dropdown').val();
			if( choosenCustomerId && choosenCustomerName ){
				if( choosenCustomerId == id && choosenCustomerName == name){
					return true;
				}else{
					console.log("This is....");
					return false;
		...
		function clearCustomer(){
			$('.customers_dropdown').attr("hiddenid", 0);
			$('.customers_dropdown').val('');
			localStorage.removeItem("choosenCustomerId");
			localStorage.removeItem("choosenCustomerName");
		}
		$('.customers_dropdown').attr('hiddenid', undefined);
		$('.customers_dropdown').focus(function(){
			// if( !isGenuineCustomer() ){
				clearCustomer();				
			// }				
		});
		$('.customers_dropdown').blur(function(){
			
			if( !isGenuineCustomer() ){
				console.log("Clear customer");
				clearCustomer();
			}
			// if( prevCustomer.id == attr && prevCustomer.id == val){
			// 	console.log("equal");
			// }else{
