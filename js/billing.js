$(document).ready(function(){
	...
	$(document).on('change', '#transaction', function(){		
		let transaction = $(this).val();
		if( transaction == "Out" ){
			// $('#fullPayment').attr("disabled", "true");
			$('#finalPendingBillAmount').val(0)
		}else{
			// $('#fullPayment').removeAttr("disabled");
		}
	...
	$(document).on('click', '#createBill', function(){		
		...
		if( customerId!=undefined && customerId!==undefined && customerId!="undefined" && customerId!=="undefined" && customerId!=0 && customerId!="0" ){
			mobile = $('#mobile').val();
			state = $('#state').val();
			city = $('#city').val();
			address = $('#address').val();
			...
			finalBillAmount = $('#finalBillAmount').val();	
			// let hamaliCharges = $('#hamaliCharges').val();
			let hamaliCharges = 0;
			...
			alert("Choose customer properly");
			// mobile = $('#mobile').val();
			// state = $('#state').val();
			// city = $('#city').val();
			// address = $('#address').val();
			...
			// gst = $('#gstno').val();
			// $.post('processing.php', {
			// 	newcustomerCreate:customername,
			// 	mobile:mobile, 
			// 	state:state, 
			// 	city:city, 
			// 	address:address, 
			...
			// }, function(res){			
			// 	if(!isNaN(res)){
			// 		$('#customername').attr('hiddenid', res)
			// 		alert("New user created, Please submit again !!");
			// 	}else{
			// 		alert("Its other than number >> "+res);
			// 	}
			// });
		}
	...
	$(document).on('click', '#updateBill', function(){		
		customername = $('#customername').val();
		customerId = $('#customername').attr('hiddenid');
		cusername = $(this).attr('data-username');
			...
			$.post('processing.php', {returnStatus:returnStatus, customername:customername, finalBillAmount:finalBillAmount, customerId:customerId, mobile:mobile, updateBillingInfos:billingInfos, transaction:transaction, oldInvoiceId:oldInvoiceId, billTotal:billTotal, billTotalQty:billTotalQty, billFinalTotal:billFinalTotal, openingBalance:openingBalance, fullPayment:fullPayment, partialPayment:partialPayment, finalPendingBillAmount:finalPendingBillAmount, hamaliCharges:hamaliCharges}, function(res){			
				if( res == true ){
					...
					if( transaction == "Out" ){
						...
						if( cusername == "retail" ){
							location.href='retailEstimate.php?estimateId='+oldInvoiceId;
						}else{
							location.href='invoice.php?invoice='+oldInvoiceId;
						}				
