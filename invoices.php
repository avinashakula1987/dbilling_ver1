					...
					<tbody>
						<?php 
							...
							if( isset($_GET['submit']) ){
								...
								$transaction = mysqli_real_escape_string($db, $_GET['transaction']);
								if( !empty($date1) && !empty($date2) ){
									if( $login == "admin" ){
										$qry = "WHERE (`date` BETWEEN '$date1' AND '$date2') AND status='1' AND transaction='$transaction'";
									}else{
										$qry = "WHERE (`date` BETWEEN '$date1' AND '$date2') AND status='1' AND login='$login' AND transaction='$transaction'";
									}										
								}else{
									if( $login == "admin" ){
										$qry = "WHERE status='1'";
								...
								if( $login == "admin" ){
									$qry = "WHERE date='".date('Y-m-d')."' AND status='1'";
								}else{
...
<script type='text/javascript'>
	$(document).ready(function() {
	    ...
	    $( api.column( 0 ).footer() ).html('Total');
			...
			$( api.column( 7 ).footer() ).html(finalTotal);
        }
